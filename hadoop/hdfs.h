...
extern  "C" {
    ...
    /**
     * hdfsRecoverFile - recover file to last sync length, release the lease
     * @param fs The configured filesystem handler.
     * @param path The path to recover
     * @return Retruns 0 on success, -1 on error
     */
    int hdfsRecoverFile(hdfsFS fs, const char *path);

    /**
     * hdfsTruncate - truncate file to new length
     * @param fs The configured filesystem handler.
     * @param path The path to recover
     * @param length New length to truncate, must in [0, file.size]
     * @return Retruns 0 on success, -1 on error
     */
    ...
    /**
     * hdfsGetFileChecksumMD5 - Get MD5 value of the File Checksum
     * file to the supplied value
     * @param fs The configured filesystem handle.
     * @param path The path of the file.
     * @return Returns NULL on error else on success.
     */
    ...
    hdfsFileInfo *hdfsGlobDirectory(hdfsFS fs, const char* path,
                                    int *numEntries);
