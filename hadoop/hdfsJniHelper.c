...
static int getTokenNumber(const char* str, char delimiter) {
...
JNIEnv* getJNIEnv(void)
{
    ...
    if (noVMs == 0) {
        ...
        int noArgs = 1;
        char jvmArgDelims[] = " ";
        const char *libhdfsOptsEnv = getenv("LIBHDFS_OPTS");
        if (libhdfsOptsEnv != NULL) {
            noArgs += getTokenNumber(libhdfsOptsEnv, jvmArgDelims[0]);
        }
        JavaVMOption options[noArgs];
        options[0].optionString = optHadoopClassPath;
        char *hadoopJvmArgs = NULL;
        if (libhdfsOptsEnv != NULL)  {
            hadoopJvmArgs = strdup(libhdfsOptsEnv);
            char *result = strtok(hadoopJvmArgs, jvmArgDelims);
            int i = 1;
            for (; result != NULL && i < noArgs; ++i) {
                options[i].optionString = result;
